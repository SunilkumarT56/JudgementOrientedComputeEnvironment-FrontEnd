...
const TestcasePanel = ({ result }: TestcasePanelProps) => {
  ...
  return (
    <div className="h-full flex flex-col bg-dark-layer-2">
      {/* Header with Tabs */}
      <div className="flex items-center gap-1 px-4 pt-2 bg-dark-layer-3 border-b border-zinc-700/50">
        <button
          onClick={() => setActiveTab('testcase')}
          className={clsx(
            "px-4 py-2 text-sm font-medium rounded-t-lg flex items-center gap-2 transition-colors relative top-[1px]",
            activeTab === 'testcase' 
              ? "text-dark-label-1 border-b-2 border-dark-brand-green" 
          ...
          <div className="space-y-4">
              ...
              {cases.map((_, i) => (
                <button
                  ...
                  className={clsx(
                    ...
                    activeCase === i
                      ? "bg-zinc-700 text-dark-label-1"
                      : "bg-transparent text-dark-label-2 hover:bg-dark-fill-2 hover:text-dark-label-1"
                  )}
                >
                  Case {i + 1}
                </button>
              ))}
              <button className="w-8 h-8 flex items-center justify-center rounded-lg text-dark-label-2 hover:bg-dark-fill-2 hover:text-dark-label-1 transition-colors">
                <Plus size={16} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="bg-zinc-700 p-3 rounded-lg font-mono text-sm text-dark-label-1 border border-transparent focus-within:border-dark-label-3 transition-colors">
                  [2,7,11,15]
                </div>
              </div>
              <div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 mb-4">
                  {cases.map((_, i) => (
                    <button
                      ...
                      onClick={() => setActiveCase(i)}
                      className={clsx(
                        "px-4 py-1.5 rounded-lg text-xs font-medium transition-colors flex items-center gap-2",
                        activeCase === i
                          ? "bg-zinc-700 text-dark-label-1"
                          : "bg-transparent text-dark-label-2 hover:bg-dark-fill-2 hover:text-dark-label-1"
                      )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="bg-zinc-700 p-3 rounded-lg font-mono text-sm text-dark-label-1">
