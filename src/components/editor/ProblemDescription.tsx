...
import { ThumbsUp, ThumbsDown, Star, Share2, MessageSquare, BookOpen, FlaskConical, Clock, CheckCircle2, Tag, ChevronRight, ChevronDown, Lock, Lightbulb, List, ChevronLeft, Maximize2, Loader2 } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        ...
        <div className="flex items-center gap-2 mb-6">
          <span className={`px-2.5 py-1 rounded-full text-sm font-medium ${
            problem.difficulty === 'Easy' ? 'text-dark-brand-green bg-dark-brand-green/10' :
            problem.difficulty === 'Medium' ? 'text-dark-brand-orange bg-dark-brand-orange/10' :
            'text-dark-brand-red bg-dark-brand-red/10'
          }`}>
            {problem.difficulty}
          </span>
        ...
        <div className="prose prose-invert prose-sm max-w-none text-dark-label-1 text-base leading-relaxed">
          <div className="whitespace-pre-wrap mb-8">
            {problem.description.split('Example 1:')[0]}
          </div>

          {problem.examples.map((example, index) => (
            <div key={index} className="my-8">
              <h3 className="text-dark-label-1 font-bold mb-4">Example {example.example_num}:</h3>
              <div className="bg-dark-fill-2 p-4 rounded-lg font-mono text-sm whitespace-pre-wrap">
